    ...
    with Session() as session:
        ...
        if user:
            ...
            try:
                ...
                update.effective_message.reply_text(f"Use /myfood to check previous records")
                try:
        ...
        if user:
            if user.foods.count():
                for _id, item in enumerate(user.foods.order_by(desc('food_time')).all()):
                    if _id < 5:
                        update.effective_message.reply_text(f"{item.id} {readable_datetime(item.food_time)} - {item.food_item.replace(',,,', ', ')}")
