...
def food(update, context):
    ...
    update.message.reply_text("Date of food? (/cancelfood to cancel)",
...
def selected_food_date(update: Update, context):
    ...
    update.callback_query.edit_message_text("Hour of food?",
...
def selected_food_hour(update: Update, context):
    ...
    update.callback_query.edit_message_text("Time of food?",
                                            reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard))
    return FOODMINUTE
...
def donephotos(update, context):
    ...
    chat_data = context.chat_data
    if not chat_data.get('food_photo_temp'):
        chat_data['photos'] = None
    else:
        chat_data['photos'] = ',,,'.join(chat_data['food_photo_temp'])
...
def save_food_record(update, context):
    ...
    with Session() as session:
        ...
        if user:
            ...
            photos = chat_data['photos']
            food_label = chat_data['food_label']
            
            print(user.id, food_time, food_item, food_notes, photos, food_label)
            food_record: Food = Food(user_id=user.id, food_time=food_time,
                                                    food_item=food_item,
                                                    food_notes=food_notes, photos=photos, food_label=food_label, created_at=datetime.now())
            try:
